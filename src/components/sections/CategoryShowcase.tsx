...
export default function CategoryShowcase({
  ...
  return (
    <div className="min-h-screen bg-brand-black text-white w-full overflow-x-hidden">
      ...
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 md:pt-44 md:pb-28 overflow-hidden border-b border-white/10">
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          ...
          <p className="text-white/70 font-light max-w-2xl text-sm md:text-base leading-relaxed mb-8">
            {description}
          </p>

          <div className="flex flex-wrap items-center gap-6 text-xs text-white/60 pt-4 border-t border-white/10">
      ...
      {/* Search & Filters */}
      <section className="py-6 bg-[#0a0a0a] border-b border-white/10 sticky top-[72px] z-40 backdrop-blur-md">
        <div className="container mx-auto px-6 md:px-12 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-none">
            <Filter className="w-4 h-4 text-brand-gold shrink-0 mr-2 hidden sm:block" />
            {families.map((fam) => (
              <button
                key={fam}
                ...
                className={`px-4 py-2 rounded-full text-xs tracking-wider uppercase whitespace-nowrap transition-all cursor-pointer ${
                  selectedFamily === fam
                    ? "bg-brand-gold text-brand-black font-semibold shadow-md shadow-brand-gold/20"
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 text-white rounded-full pl-10 pr-4 py-2 text-xs placeholder:text-white/40 focus:outline-none focus:border-brand-gold transition-colors"
            />
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-brand-black">
        <div className="container mx-auto px-6 md:px-12">
          ...
          {filteredProducts.length === 0 ? (
            <div className="text-center py-20 bg-white/[0.01] border border-white/5 rounded-xl">
